...
def computeBrief(im, gaussian_pyramid, locsDoG, compareX, compareY):
    ...
    patch_width = 9  # Size of the patch
    n = compareX.shape[0] # number of bits
    locs = []
    for i in range(locsDoG.shape[0]):
        keypoint = locsDoG[i, :] 
        x = keypoint[1] # x coordinate
        y = keypoint[0] # y coordinate
        if (x >= patch_width//2) and (y >= patch_width//2) and (x< im.shape[0]-patch_width//2) and (y < - patch_width//2 + im.shape[1]):
            locs.append(locsDoG[i,:])
    locs = np.array(locs) # to make it sure that is is a numpy array

    desc = np.empty((0, n))
    for j in range(locsDoG.shape[0]):
        keypoint = locsDoG[j, :]
